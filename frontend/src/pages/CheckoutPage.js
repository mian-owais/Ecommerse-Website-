...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (!cart || !cart.items || !Array.isArray(cart.items) || cart.items.length === 0) {
      console.error('‚ùå Cart is empty or invalid:', cart);
      setError('‚ùå Your cart is empty. Add items before placing an order.');
      return;
    }

    console.log('‚úì Cart verified with', cart.items.length, 'items');
    ...
    try {
      ...
      if (response.success) {
        ...
        if (onlinePaymentMethods.includes(formData.paymentMethod)) {
          ...
          setSuccess('‚úì Order created. Now complete the payment.');
        } else {
          // COD - directly complete the order
          setSuccess('‚úì Order placed successfully!');
          setTimeout(() => {
        ...
        setError('‚ùå ' + (response.message || 'Failed to place order. Please try again.'));
      }
    } catch (err) {
      console.error('Order creation error:', err);
      setError('‚ùå ' + (err.message || 'Failed to place order. Please try again.'));
    } finally {
  ...
  const handlePaymentComplete = () => {
    ...
    setSuccess('‚úì Payment verified! Order confirmed.');
              ...
              <div className="payment-methods">
                <label className="payment-option">
                  <input
                    ...
                    onChange={handleInputChange}
                  />
                  <span>Ì≤µ Cash on Delivery (COD)</span>
                </label>
                <label className="payment-option">
                  <input
                    type="radio"
                    ...
                    onChange={handleInputChange}
                  />
                  <span>Ì≤≥ Credit Card (Stripe)</span>
                </label>
                <label className="payment-option">
                  <input
                    type="radio"
